...
assert.match(index,/realtime-state\.js\?v=90/);
assert.match(index,/TEST v90 · default context mobile search/);
...
assert.match(reportHtml,/report\.js\?v=90/);
...
assert.deepEqual(readmeVersions,[90,89,88]);
...
assert.match(index,/js\/features\/smart-context-search\.js\?v=90/);
assert.match(fs.readFileSync(path.join(root,'css/main.css'),'utf8'),/features\/smart-context-search\.css\?v=90/);
...
assert.match(smartSearchCss,/grid-template-columns:repeat\(6,minmax\(0,1fr\)\)!important/);
assert.match(smartSearchCss,/\.profile-card>\.dashboard-tabs \.smart-search-nav-button\{display:flex!important\}/);
...
assert.doesNotMatch(usafMarkup,/usaf-law-group-head-v90"><span>0[1-8]/);
assert.doesNotMatch(usafMarkup,/Четыре правила из материалов National Guard/);
assert.ok(index.indexOf('data/usaf/law-guide.js?v=90')>index.indexOf('data/usaf/content.js?v=90'));
assert.ok(index.indexOf('data/usaf/law-guide.js?v=90')<index.indexOf('data/usaf/tests.js?v=90'));
...
assert.match(surfaceState,/rpCabinetOpenDetails_v90/);
...
assert.match(lawCss,/\.usaf-law-group-head-v90\{[^}]*border:0/);
assert.match(lawCss,/\.usaf-law-flow-v90 li\{[^}]*flex-direction:column/);
assert.doesNotMatch(lawCss,/grid-template-columns:minmax\(160px/);

console.log('RP CABINET v90 default context mobile search, law layout and realtime tests: OK');

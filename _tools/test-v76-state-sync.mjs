...
assert.match(app,/CLOUD_PROFILE_ID='default_v77'/);
assert.match(app,/PROFILE_CONTEXT_CHECKPOINT_KEY='kiri:rp-cabinet:v77:context-checkpoint'/);
assert.match(storage,/rp_panel_account_profile_V77_STABLE/);
assert.match(version,/TEST_VERSION="76"/);
assert.match(html,/TEST v77 · Isolated state sync/);
assert.ok(!html.includes('?v=75'));
console.log('v77 state-sync checks passed');

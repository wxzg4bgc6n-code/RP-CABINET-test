...
assert.match(index,/realtime-state\.js\?v=91/);
assert.match(index,/TEST v91 · responsive ownership and visible search filters/);
...
assert.match(reportHtml,/report\.js\?v=91/);
...
assert.deepEqual(readmeVersions,[91,90,89]);
...
assert.match(index,/js\/features\/smart-context-search\.js\?v=91/);
assert.match(mainCss,/features\/smart-context-search\.css\?v=91/);
...
assert.ok(mainCss.indexOf('features/smart-context-search.css?v=91')<mainCss.indexOf('responsive/mobile.css?v=91'));
...
assert.doesNotMatch(usafMarkup,/usaf-law-group-head-v91"><span>0[1-8]/);
assert.doesNotMatch(usafMarkup,/Четыре правила из материалов National Guard/);
assert.ok(index.indexOf('data/usaf/law-guide.js?v=91')>index.indexOf('data/usaf/content.js?v=91'));
assert.ok(index.indexOf('data/usaf/law-guide.js?v=91')<index.indexOf('data/usaf/tests.js?v=91'));
...
assert.match(surfaceState,/rpCabinetOpenDetails_v91/);
...
assert.match(lawCss,/\.usaf-law-group-head-v91\{[^}]*border:0/);
assert.match(lawCss,/\.usaf-law-flow-v91 li\{[^}]*flex-direction:column/);
assert.doesNotMatch(lawCss,/grid-template-columns:minmax\(160px/);

console.log('RP CABINET v91 responsive ownership and visible search filters, law layout and realtime tests: OK');

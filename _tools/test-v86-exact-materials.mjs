...
assert.match(index,/realtime-state\.js\?v=86/);
assert.match(index,/TEST v86 · exact promotion materials/);
...
assert.deepEqual(readmeVersions,[86,85,84]);
...
assert.doesNotMatch(usafMarkup,/usaf-law-group-head-v86"><span>0[1-8]/);
assert.doesNotMatch(usafMarkup,/Четыре правила из материалов National Guard/);
assert.ok(index.indexOf('data/usaf/law-guide.js?v=86')>index.indexOf('data/usaf/content.js?v=86'));
assert.ok(index.indexOf('data/usaf/law-guide.js?v=86')<index.indexOf('data/usaf/tests.js?v=86'));
...
assert.match(surfaceState,/rpCabinetOpenDetails_v86/);
...
assert.match(lawCss,/\.usaf-law-group-head-v86\{[^}]*border:0/);
assert.match(lawCss,/\.usaf-law-flow-v86 li\{[^}]*flex-direction:column/);
assert.doesNotMatch(lawCss,/grid-template-columns:minmax\(160px/);

console.log('RP CABINET v86 exact promotion materials, law layout and realtime tests: OK');

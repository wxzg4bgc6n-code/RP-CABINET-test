...
assert.match(index,/TEST v103 · Browser icon/);
assert.match(index,/modular-v103-browser-icon/);
for(const html of [index,reportHtml]){
  assert.match(html,/rel="icon" href="favicon\.ico" sizes="any"/);
  assert.match(html,/assets\/icons\/favicon\.svg\?v=103/);
  assert.match(html,/assets\/icons\/favicon-32\.png\?v=103/);
  assert.match(html,/assets\/icons\/apple-touch-icon\.png\?v=103/);
  assert.match(html,/site\.webmanifest\?v=103/);
}
for(const icon of [
  'favicon.ico',
...
assert.ok(index.indexOf('js/config/google-drive.js?v=103')<index.indexOf('js/services/google-drive-storage.js?v=103'));
assert.ok(index.indexOf('js/services/google-drive-storage.js?v=103')<index.indexOf('js/app.js?v=103'));
assert.ok(index.indexOf('js/app.js?v=103')<index.indexOf('js/features/google-drive-avatar.js?v=103'));
assert.match(mainCss,/features\/google-drive-avatar\.css\?v=103/);
assert.ok(index.indexOf('data/usaf/law-data.js?v=103')<index.indexOf('data/usaf/law-guide.js?v=103'));
assert.ok(index.indexOf('data/usaf/tests.js?v=103')<index.indexOf('data/usaf/law-exam.js?v=103'));
assert.match(sidebarPromo,/https:\/\/github\.com\/wxzg4bgc6n-code\/RP-CABINET-test\/releases\/download\/program-v1\.0\.0\/RP-Helper\.zip/);
assert.match(sidebarPromo,/download="RP-Helper\.zip"/);
assert.match(sidebarPromo,/<strong>RP-Helper<\/strong>/);
...
assert.match(reportHtml,/js\/config\/google-drive\.js\?v=103/);
...
assert.deepEqual(versionHeadings,['103','102','101']);
assert.match(readme,/Папки `proof-service` в v103 нет/);
...
console.log('RP CABINET v103 Browser icon, USAF exam and realtime tests: OK');

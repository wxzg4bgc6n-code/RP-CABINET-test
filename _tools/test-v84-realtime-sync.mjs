...
assert.match(index,/realtime-state\.js\?v=84/);
assert.match(index,/TEST v84 · USAF promotion materials/);
...
assert.doesNotMatch(usafMarkup,/usaf-law-group-head-v84"><span>0[1-8]/);
assert.doesNotMatch(usafMarkup,/Четыре правила из материалов National Guard/);
assert.ok(index.indexOf('data/usaf/law-guide.js?v=84')>index.indexOf('data/usaf/content.js?v=84'));
assert.ok(index.indexOf('data/usaf/law-guide.js?v=84')<index.indexOf('data/usaf/tests.js?v=84'));
const pinnedMaterials=fs.readFileSync(path.join(root,'js/features/pinned-materials.js'),'utf8');
assert.match(pinnedMaterials,/return \["usaf-law", "usaf-posts"/);
assert.match(pinnedMaterials,/tasks\.indexOf\("уак"\)/);
assert.doesNotMatch(pinnedMaterials,/\["usaf-posts", "usaf-patrols", "usaf-test-aircraft", "usaf-test-mp-uak-pk"/);
const surfaceState=fs.readFileSync(path.join(root,'js/features/surface-state.js'),'utf8');
assert.match(surfaceState,/rpCabinetOpenDetails_v84/);
...
console.log('RP CABINET v84 promotion materials, open-state and realtime tests: OK');

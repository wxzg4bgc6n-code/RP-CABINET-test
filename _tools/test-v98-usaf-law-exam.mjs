...
assert.match(index,/TEST v98 · USAF UAK\/PK guide and exam/);
assert.match(index,/modular-v98-usaf-law-exam/);
assert.ok(index.indexOf('js/config/google-drive.js?v=98')<index.indexOf('js/services/google-drive-storage.js?v=98'));
assert.ok(index.indexOf('js/services/google-drive-storage.js?v=98')<index.indexOf('js/app.js?v=98'));
assert.ok(index.indexOf('js/app.js?v=98')<index.indexOf('js/features/google-drive-avatar.js?v=98'));
assert.match(mainCss,/features\/google-drive-avatar\.css\?v=98/);
assert.ok(index.indexOf('data/usaf/law-data.js?v=98')<index.indexOf('data/usaf/law-guide.js?v=98'));
assert.ok(index.indexOf('data/usaf/tests.js?v=98')<index.indexOf('data/usaf/law-exam.js?v=98'));
...
assert.match(reportHtml,/js\/config\/google-drive\.js\?v=98/);
...
assert.deepEqual(versionHeadings,['98','97','96']);
assert.match(readme,/Папки `proof-service` в v98 нет/);
...
assert.equal(lawRuntime.window.RP_USAF_LAW_EXAM.questions.length,20);
assert.equal(lawRuntime.window.RP_USAF_LAW_EXAM.questions[0].options[3],'1 час');
assert.equal(lawRuntime.window.RP_USAF_LAW_EXAM.questions[17].options.length,10);
assert.deepEqual(Array.from(lawRuntime.window.RP_USAF_LAW_EXAM.questions[17].correct),[0,1,2,8,9]);
assert.equal(lawRuntime.window.RP_USAF_LAW_EXAM.questions[19].options[2],'1 год лишения свободы');
...
assert.equal((patchedTests.match(/usaf-law-exam-question-v98/g)||[]).length,20);
...
console.log('RP CABINET v98 USAF UAK/PK exam, Drive and realtime tests: OK');

...
assert.match(index,/realtime-state\.js\?v=87/);
assert.match(index,/TEST v87 · report loading dividers/);
...
assert.match(reportHtml,/report\.js\?v=87/);
...
assert.deepEqual(readmeVersions,[87,86,85]);
...
assert.doesNotMatch(usafMarkup,/usaf-law-group-head-v87"><span>0[1-8]/);
assert.doesNotMatch(usafMarkup,/Четыре правила из материалов National Guard/);
assert.ok(index.indexOf('data/usaf/law-guide.js?v=87')>index.indexOf('data/usaf/content.js?v=87'));
assert.ok(index.indexOf('data/usaf/law-guide.js?v=87')<index.indexOf('data/usaf/tests.js?v=87'));
...
assert.match(surfaceState,/rpCabinetOpenDetails_v87/);
...
assert.match(lawCss,/\.usaf-law-group-head-v87\{[^}]*border:0/);
assert.match(lawCss,/\.usaf-law-flow-v87 li\{[^}]*flex-direction:column/);
assert.doesNotMatch(lawCss,/grid-template-columns:minmax\(160px/);

console.log('RP CABINET v87 report loading dividers, law layout and realtime tests: OK');

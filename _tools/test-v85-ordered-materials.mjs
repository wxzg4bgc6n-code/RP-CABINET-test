...
assert.match(index,/realtime-state\.js\?v=85/);
assert.match(index,/TEST v85 · ordered promotion materials/);
...
assert.doesNotMatch(usafMarkup,/usaf-law-group-head-v85"><span>0[1-8]/);
assert.doesNotMatch(usafMarkup,/Четыре правила из материалов National Guard/);
assert.ok(index.indexOf('data/usaf/law-guide.js?v=85')>index.indexOf('data/usaf/content.js?v=85'));
assert.ok(index.indexOf('data/usaf/law-guide.js?v=85')<index.indexOf('data/usaf/tests.js?v=85'));
...
assert.match(pinnedMaterials,/name\.indexOf\("frogger"\).*return "usaf-test-aircraft"/);
...
assert.match(surfaceState,/rpCabinetOpenDetails_v85/);
...
assert.match(lawCss,/\.usaf-law-group-head-v85\{[^}]*border:0/);
assert.match(lawCss,/\.usaf-law-flow-v85 li\{[^}]*flex-direction:column/);
assert.doesNotMatch(lawCss,/grid-template-columns:minmax\(160px/);

console.log('RP CABINET v85 ordered promotion materials, law layout and realtime tests: OK');

...
assert.match(index,/realtime-state\.js\?v=88/);
assert.match(index,/TEST v88 · report avatar loader fix/);
...
assert.match(reportHtml,/report\.js\?v=88/);
...
assert.deepEqual(readmeVersions,[88,87,86]);
...
assert.doesNotMatch(usafMarkup,/usaf-law-group-head-v88"><span>0[1-8]/);
assert.doesNotMatch(usafMarkup,/Четыре правила из материалов National Guard/);
assert.ok(index.indexOf('data/usaf/law-guide.js?v=88')>index.indexOf('data/usaf/content.js?v=88'));
assert.ok(index.indexOf('data/usaf/law-guide.js?v=88')<index.indexOf('data/usaf/tests.js?v=88'));
...
assert.match(surfaceState,/rpCabinetOpenDetails_v88/);
...
assert.match(lawCss,/\.usaf-law-group-head-v88\{[^}]*border:0/);
assert.match(lawCss,/\.usaf-law-flow-v88 li\{[^}]*flex-direction:column/);
assert.doesNotMatch(lawCss,/grid-template-columns:minmax\(160px/);

console.log('RP CABINET v88 report avatar loader fix, law layout and realtime tests: OK');

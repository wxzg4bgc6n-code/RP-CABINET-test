...
if(!version.includes('TEST_VERSION="108"')) failures.push('TEST_VERSION is not 108');
if(!version.includes('Rules & reports refresh')) failures.push('TEST_VERSION_LABEL mismatch');
if(!index.includes('TEST v108 · Rules & reports refresh')) failures.push('visible badge mismatch');
if(!coreCss.includes('TEST v108 · Rules & reports refresh')) failures.push('CSS badge mismatch');
if(!index.includes('modular-v108-rules-reports')) failures.push('architecture marker mismatch');
...
if(!index.includes('js/features/onboarding.js?v=108')) failures.push('onboarding JS missing');
if(!mainCss.includes('features/onboarding.css?v=108')) failures.push('onboarding CSS missing');
...
for(const html of [index,report]){
  ...
  if(!html.includes('js/config/google-drive-public-key.js?v=108')) failures.push('public key helper missing');
  if(html.indexOf('public-config.js')>html.indexOf('google-drive-public-key.js')) failures.push('public config loads too late');
  if(!html.includes('assets/icons/favicon.svg?v=108')) failures.push('SVG favicon missing');
  if(!html.includes('site.webmanifest?v=108')) failures.push('manifest missing');
}
if(!mainCss.includes('features/progress-proofs.css?v=108')) failures.push('progress proof CSS missing');
if(!index.includes('js/features/progress-proofs.js?v=108')) failures.push('progress proof JS missing');
...
console.log('v108 build audit passed');

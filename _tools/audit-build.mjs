...
if(!index.includes('TEST v97 · Proof cleanup and bulk delete')) failures.push('index badge is not v97');
if(!version.includes('const TEST_VERSION="97"')) failures.push('TEST_VERSION is not 97');
if(!version.includes('Proof cleanup and bulk delete')) failures.push('TEST_VERSION_LABEL mismatch');
if(!coreCss.includes('TEST v97 · Proof cleanup and bulk delete')) failures.push('CSS badge mismatch');
if(!index.includes('modular-v97-proof-cleanup')) failures.push('Architecture meta marker mismatch');
if(index.includes('sync-merge.js')) failures.push('Old sync-merge.js is still connected');
if(!index.includes('realtime-state.js?v=97')) failures.push('realtime-state.js is not connected with v97 cache-busting');
...
if(!index.includes('data/usaf/law-guide.js?v=97')) failures.push('USAF law guide is not connected');
if(!mainCss.includes('features/usaf-law.css?v=97')) failures.push('USAF law guide styles are not connected');
if(!index.includes('features/smart-context-search.js?v=97')) failures.push('Smart search script is not connected');
if(!mainCss.includes('features/smart-context-search.css?v=97')) failures.push('Smart search styles are not connected');
if(!mainCss.includes('responsive/mobile.css?v=97')) failures.push('Mobile styles are not connected');
if(mainCss.indexOf('features/smart-context-search.css?v=97')>mainCss.indexOf('responsive/mobile.css?v=97')) failures.push('Mobile styles must load after common smart-search styles');
...
if(!index.includes('js/config/google-drive.js?v=97')) failures.push('Google Drive config is not connected');
if(!index.includes('js/services/google-drive-storage.js?v=97')) failures.push('Google Drive storage service is not connected');
if(!index.includes('js/features/google-drive-avatar.js?v=97')) failures.push('Google Drive avatar feature is not connected');
if(!mainCss.includes('features/google-drive-avatar.css?v=97')) failures.push('Google Drive avatar styles are not connected');
if(index.indexOf('js/config/google-drive.js?v=97')>index.indexOf('js/services/google-drive-storage.js?v=97')) failures.push('Drive config must load before Drive service');
if(index.indexOf('js/services/google-drive-storage.js?v=97')>index.indexOf('js/app.js?v=97')) failures.push('Drive service must load before app.js');
...
console.log(`RP CABINET v97 build audit: OK\n${notes.join('\n')}`);

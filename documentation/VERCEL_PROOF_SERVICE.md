...
1. Загрузить всё содержимое папки v92 в корень GitHub-репозитория, сохранив структуру.
...
8. убедиться, что индикатор загрузки исчез после появления отчёта;
9. проверить Google-аватар профиля в верхней шапке либо инициалы, если фотографии нет;
10. удалить тестовый отчёт.
...
- новые отчёты сохраняют только HTTPS-ссылку на Google-аватар; само изображение профиля в Blob не копируется;
- старые отчёты без `profile.avatarUrl` продолжают открываться и показывают инициалы;
- `report.html` делает две попытки чтения по 10 секунд и не оставляет бесконечный индикатор.
